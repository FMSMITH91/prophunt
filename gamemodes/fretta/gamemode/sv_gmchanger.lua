...
fretta_votesneeded = CreateConVar( "fretta_votesneeded", "0.55", { FCVAR_ARCHIVE } )
...
function GM:VoteForChange( ply )
	...
	if ( CurTime() < GetConVarNumber( "fretta_votegraceperiod" ) ) then -- can't vote too early on
		...
		BroadcastLua( "chat.AddText( Entity("..ply:EntIndex().."), Color( 255, 255, 255 ), [[ voted to change the map]] )" )
	else
