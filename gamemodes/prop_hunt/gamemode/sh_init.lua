...
-- Server only. engine.GetGamemodes() reports the gamemodes installed on the
-- machine it runs on, so running this clientside told players about their own
-- local installs - which say nothing about the server they just joined - and
-- threw a red Lua error at them on every connect.
if SERVER then
	for _,data in ipairs( engine.GetGamemodes() ) do
